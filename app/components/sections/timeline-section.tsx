...
export function TimelineSection({ items }: TimelineSectionProps) {
  return (
    <section className="py-24 md:py-32 bg-zinc-50 dark:bg-zinc-950/50" aria-labelledby="timeline-heading">
      <Container>
        <Reveal>
          <SectionHeading
            label="Perjalanan"
          ...
          <div className="flex gap-6 overflow-x-auto pb-6 snap-x snap-mandatory scrollbar-thin">
            {items.map((item, i) => (
              <Reveal key={item.id} delay={i * 0.08}>
                <article className="w-72 shrink-0 snap-start rounded-2xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
